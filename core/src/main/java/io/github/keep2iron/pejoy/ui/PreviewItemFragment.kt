...
class PreviewItemFragment : Fragment(), View.OnClickListener {
  ...
  override fun onCreateView(
    ...
    val content =
      inflater.inflate(R.layout.pejoy_fragment_preview_item, container, false) as ViewGroup
    val layoutParams =
  ...
  override fun onViewCreated(
    ...
    when {
      ...
      item.isGif -> engine.loadGifImage(
          requireContext(), size.x, size.y, placeholder, imageView, item.contentUri
      )
      else -> engine.loadImage(
          requireContext(), size.x, size.y, placeholder, imageView, item.contentUri
      )
    }
  }
