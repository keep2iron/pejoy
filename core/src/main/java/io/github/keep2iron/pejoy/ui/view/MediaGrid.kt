...
class MediaGrid @JvmOverloads constructor(
  ...
  private fun init(context: Context) {
    LayoutInflater.from(context)
        .inflate(R.layout.pejoy_widget_media_grid, this, true)

    mThumbnail = SelectionSpec.instance.requireImageEngine()
        .provideImageView(context)
    ...
    if (mListener != null) {
      if (v === mThumbnail) {
        mListener!!.onThumbnailClicked(
            mThumbnail, media, mPreBindInfo.mViewHolder, mPreBindInfo.mPosition
        )
      } else if (v === mCheckView) {
        mListener!!.onCheckViewClicked(
            mCheckView, media, mPreBindInfo.mViewHolder, mPreBindInfo.mPosition
        )
      }
    }
  }
  ...
  private fun setImage() {
    if (media.isGif) {
      SelectionSpec.instance.requireImageEngine()
          .loadGifThumbnail(
